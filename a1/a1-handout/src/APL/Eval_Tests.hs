...
tests =
  testGroup
    ...
    [ testCase "Add" $
      ...
      --
      testCase "Lambda" $
        eval envEmpty (Let "x" (CstInt 2) (Lambda "y" (Add (CstInt 2) (CstInt 4))))
        @?= Right (ValFun [("x", ValInt 2)] "y" (Add (CstInt 2) (CstInt 4))),
      --
      testCase "Apply" $
        eval envEmpty (Apply (Let "x" (CstInt 2) (Lambda "y" (Add (Var "x") (Var "y")))) (CstInt 3))
        @?= Right (ValInt 5),
      --
      testCase "Apply Without using variables" $
        eval envEmpty (Apply (Let "x" (CstInt 2) (Lambda "y" (Add (CstInt 2) (CstInt 4)))) (CstInt 3))
        @?= Right (ValInt 6),
      --
      testCase "Lambda + Apply Exstensive" $
        eval envEmpty (Let "x" (CstInt 2) (Apply (Let "y" (CstInt 10) (Lambda "x" (Add (Var "x") (Var "y")))) (Add (Var "x") (Var "x"))))
        @?= Right (ValInt 14),
      --
      testCase "TryCatch Try Fails" $
        eval envEmpty (TryCatch (Div (CstInt 2) (CstInt 0)) (CstInt 1))
        @?= Right (ValInt 1),
      --
      testCase "TryCatch Try Succeeds" $
        eval envEmpty (TryCatch (Div (CstInt 2) (CstInt 1)) (CstInt 1))
        @?= Right (ValInt 2),
      --
      testCase "TryCatch Try and Catch Fails" $
        eval envEmpty (TryCatch (Div (CstInt 2) (CstInt 0)) (Div (CstInt 2) (CstInt 0)))
        @?= Left "Both cases of Try Catch failed"
          --
          -- TODO - add more
    ]
