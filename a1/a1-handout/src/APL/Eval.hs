...
eval env (Lambda var e1) =
  case eval env e1 of
    Left err -> if err == ("Unknown variable: " ++ var) 
      then Right $ ValFun env var e1
      else Left err
    Right _ -> Right $ ValFun env var e1
eval env (Apply e1 e2) =
  case (eval env e1, eval env e2) of
    (Right (ValFun _ _ _), Right (ValFun _ _ _)) -> Left "Cannot be used as variable value"
    (Right (ValFun oldEnv var e1'), Right v) -> eval (envExtend var v oldEnv) e1'
    (Left err, _) -> Left err
    (_, Left err) -> Left err
    (_, _) -> Left "Nothing to apply"
eval env (TryCatch e1 e2) =
  case (eval env e1, eval env e2) of
    (Left _, Right x) -> Right x
    (Right x, _) -> Right x
    (_, _) -> Left "Both cases of Try Catch failed"
